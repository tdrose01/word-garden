...
The play screen fits the complete crossword alongside the word wheel. Cells adapt to puzzle dimensions with at least 18px letter text: ordinary boards receive larger cells, while wide 13-column boards use 24px cells on compact phones (22px on 320px screens). The optional enlarged board remains available. Wheel touch targets are at least 44px and stay separated when selected. Browser validation checks every campaign shape on a compact phone, plus Level 5 gameplay after clues and swipes across three phone sizes.
...
Wheel diameter grows with the number of letters so touch targets do not overlap. Compact status rows and controls alongside the wheel reserve space for the entire board. Unusually short screens may extend the page to preserve readable cells, but the grid itself is never a small panning window. Guidance text uses opaque high-contrast backing across all scenery.
